...
Forward Factor definition (Campasano / Sean Ryan):
    σ_fwd = sqrt(max(0, (T2*σ2² - T1*σ1²) / (T2-T1)))
    FF = (front_iv - σ_fwd) / front_iv * 100
...
OPTTRADER_DIR = Path.home() / "Documents" / "OptionTrader"
...
FRONT_DTE_TARGET = 30
BACK_DTE_TARGET = 60
FRONT_DTE_WINDOW = (20, 45)   # acceptable DTE range for front
BACK_DTE_WINDOW = (45, 80)    # acceptable DTE range for back
...
def forward_vol(front_iv: float, front_t: float, back_iv: float, back_t: float) -> float:
    """Compute annualized forward volatility between two expirations."""
...
def ff_to_decile(ff: float | None) -> int | None:
    """Approximate FF decile (exact decile requires full cross-section)."""
    if ff is None:
        return None
    if ff < 2:
        return 1
...
def fetch_chain(ticker: str, dte: int, no_ib: bool) -> dict | None:
    """
    Call option_chain.py for given ticker/DTE.
    Returns: {spot, expiry, dte, iv, strike, delta, volume, bid_ask_spread}
    """
    cmd = [
    ...
    # OptionTrader uses "C" / "P" for right
    calls = [
        r for r in rows
    ...
    def atm_key(r):
        ...
        if d is not None:
            return abs(d - 0.50)
        # Fallback: strike closest to spot
    ...
    # Volume: sum across all rows at this expiry (call + put)
...
def scan_ticker(ticker: str, no_ib: bool) -> dict | None:
    ...
    if front.get("expiry") == back.get("expiry"):
        return None  # same expiry — can't form a calendar
...
def load_universe_from_file(path: str) -> list[str]:
    """Load ticker list from ff_universe_scan.py candidates CSV."""
    import csv as _csv
