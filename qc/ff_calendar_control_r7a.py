...
"""
R7a harness validation + positive control (pre-registered 2026-09-14,
RESEARCH_PLAN.md Test R7a). Gate: mid (k=0) EV >= +10% with 95% CI excluding
0 on the replica population -- else STOP, no R7 kill is licensed.

Base: COPIED from qc/ff_calendar_automation_r61.py (base untouched). Same
entry worker (mid-pegged debit ComboLimitOrder, 3x10min walk to
fill_aggressive, 30min max-wait skip, buy-first fallback, TP40 + front-1d
exits, regular-hours Limit assignment flatten, Tastytrade model) and same
slim harvest contract (CSFFLOG filled + CSFFCLOSE + SUMMARY/SKIPTOT).
Replica deltas vs R6.1 (all toward the Dolt-era 177k conditions, ledger row 1):
  (a) universe = R5.1 70-name census set (20 ETFs + 50 stocks), not locked 20;
  (b) DTE bands 20-45/46-80 with FIRST expiry in band (ff_full_backtest.py
      find_expirations), not 20-40/45-75 closest-to-30/60.
Known remaining deltas vs Dolt (documented, not closed): minute NBBO mids vs
EOD mids; put calendar vs straddle calendar (EV% invariant at ATM by
put-call parity -- ff_ml_v2.py build_features_put); put-mid BS-inversion FF
vs call-IV FF; 15:15 ET entry vs daily EOD observation; 2024-01->2026-09
window (QC minute-history depth) vs 2020-2025; TP40+front-1d exits
(Dolt-validated best exit, +3.5pp over hold -- conservative FOR the control).
Windows run as entry-partitioned quarterly runs (same 11 as R6.1); harvest
concatenates identically. Output: results/r7a_control.csv.
"""


class FFCalendarControlR7A(QCAlgorithm):

    # ---- R7a replica universe: R5.1 70-name census set (20 ETFs + 50 stocks) ----
    # Source: analysis/2026-09-11_subagent-csff-a2-census.md §Universe source.
    U70 = ["XLY", "KRE", "XLV", "XLC", "XLE", "XLI", "XLU", "XLK", "SPY",
           "XLB", "XLP", "XLF", "XLRE", "XME", "XOP", "XRT", "SMH", "QQQ",
           "IWM", "DIA",
           "AAPL", "NVDA", "TSLA", "AMD", "AMZN", "META", "MSFT", "GOOGL",
           "NFLX", "PLTR", "COIN", "MSTR", "MARA", "SOFI", "NIO", "PYPL",
           "INTC", "MU", "AVGO", "ORCL", "CRM", "ADBE", "SHOP", "SQ", "UBER",
           "JPM", "BAC", "GS", "MS", "WFC", "XOM", "CVX", "F", "GM", "BA",
           "DIS", "T", "WMT", "HD", "NKE", "SBUX", "PFE", "MRNA", "LLY",
           "UNH", "COST", "AAL", "UAL", "HOOD", "SMCI"]
    TICKERS = list(U70)

    FF_MIN = 20.0
    # Dolt-era bands (ff_full_backtest.py:93-94) + first-in-band pick (:182-196)
    FRONT_LO, FRONT_HI = 20, 45
    BACK_LO, BACK_HI = 46, 80
    TP_MULT = 1.40
    CLOSE_DTE = 1
    RISK_FREE = 0.04
    ...
    def Initialize(self):
        # R7a window params: QC cloud keeps only ~100KB of backtest log per
        ...
        # sharding: 70 names x minute options may be too heavy for one
        # backtest (R5.1 census used 4 shards for scan-only) -- params
        # shard/nshards partition U70 round-robin; ObjectStore keys carry
        # the shard suffix. Pilot unsharded first; shard on OOM/timeout.
        self.shard = int(self.GetParameter("shard", "0"))
        self.nshards = max(1, int(self.GetParameter("nshards", "1")))
        self.TICKERS = [t for i, t in enumerate(self.U70)
        ...
        # use the shard's first name (same US-equity hours for all 70).
    ...
    def _pick(self, chain, spot):
        ...
        # Dolt-era pick (ff_full_backtest.py find_expirations): FIRST expiry
        # in each DTE band (not closest-to-target).
        def band(lo, hi):
            exps = sorted({c.Expiry.date() for c in puts
                           if lo <= (c.Expiry.date() - today).days <= hi})
            return exps[0] if exps else None

        fe = band(self.FRONT_LO, self.FRONT_HI)
        be = band(self.BACK_LO, self.BACK_HI)
    ...
    def OnEndOfAlgorithm(self):
        ...
        if self.attempt_rows:
            ...
            self.ObjectStore.Save(f"ff_r7a_attempts_{suffix}", csv)
        if self.trade_rows:
            cols = list(self.trade_rows[0].keys())
            csv = ",".join(cols) + "\n" + "\n".join(
                ",".join(str(r.get(c, "")) for c in cols)
                for r in self.trade_rows)
            self.ObjectStore.Save(f"ff_r7a_trades_{suffix}", csv)
